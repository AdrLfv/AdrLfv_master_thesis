...
One of the primary aims of the augmented mirror is to enable learning by interacting in augmented reality. It offers several applications that combine entertainment and practice. The user can choose and launch an application from the mirror's menu. In the sign language game application, the user must read and follow the instructions given by Aria to complete the adventure. The game is particularly suitable for stimulating practice in children.
The mirror doesn't require any additional equipment, but it does need a certain amount of space to be used. The user must be able to stand two meters away from the device and make various movements without being disturbed. The mirror is also not suitable for use in a dark room. The user must be in a well-lit room to be detected by the camera.
...
Some users have experienced problems related to poor AI recognition. One user took 8:25 to complete the game (against 4.44 minutes on average). He had difficulty making the "go" sign in the house. The high difference (1,84m for the dataset creator against 1,60m for the user) explains the bias. Users were tested after their session on some random signs. Their average score was 2.33 signs learned out of 4. The errors made are related to several elements. The AI sometimes finds a sign too quickly when the user still needs to place his hands correctly. 
...
The stimulation of three different intelligences, added to the motivating gamification aspect, favors information retention. The user better remembers a particular gesture by having seen it, projected it mentally, and by having performed it physically in space. Indeed, it has been proven that certain non-verbal behaviors, such as gestures, can have a powerful impact on the understanding and memorization of communication \cite{kelly1999offering}.
